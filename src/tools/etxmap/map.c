...
static qboolean ParseMapEntity(qboolean onlyLights)
{
	...
	if(convertType == CONVERT_QUAKE3)
	{
		...
		// or the detail bruhs flags will be lost and the brushes will generate really many bsp splits
		#if 1
		if(numEntities == 1)
		{
			brush_t        *b, *prev, *next;
			...
			memset(&convertDetailBrushesFuncStaticEntity, 0, sizeof(convertDetailBrushesFuncStaticEntity));

			SetKeyValue(&convertDetailBrushesFuncStaticEntity, "classname", "func_static");
			SetKeyValue(&convertDetailBrushesFuncStaticEntity, "name", "worldspawn_detail_brushes_0");
			SetKeyValue(&convertDetailBrushesFuncStaticEntity, "model", "worldspawn_detail_brushes_0");
			SetKeyValue(&convertDetailBrushesFuncStaticEntity, "description", "Generated by ETXMap (XreaL)");
			...
			// add structural brushes to worldspawn
			...
			while(detailBrushes)
			{
				...
				// link opaque brushes to head of list, translucent brushes to end
				if(b->opaque || convertDetailBrushesFuncStaticEntity.lastBrush == NULL)
				{
					b->next = convertDetailBrushesFuncStaticEntity.brushes;
					convertDetailBrushesFuncStaticEntity.brushes = b;
					if(convertDetailBrushesFuncStaticEntity.lastBrush == NULL)
						convertDetailBrushesFuncStaticEntity.lastBrush = b;
				}
				else
				{
					b->next = NULL;
					convertDetailBrushesFuncStaticEntity.lastBrush->next = b;
					convertDetailBrushesFuncStaticEntity.lastBrush = b;
				}
				...
				if(b->contentShader != NULL && b->contentShader->colorMod != NULL && b->contentShader->colorMod->type == CM_VOLUME)
				{
					b->nextColorModBrush = convertDetailBrushesFuncStaticEntity.colorModBrushes;
					convertDetailBrushesFuncStaticEntity.colorModBrushes = b;
				}
