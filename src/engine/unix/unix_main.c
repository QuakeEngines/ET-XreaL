...
=================
*/
qboolean CopyDLLForMod(char **p_fn, const char *gamedir, const char *pwdpath, const char *homepath, const char *basepath,
					   const char *fname)
{
	char           *fn = *p_fn;
	...
	if(strlen(gamedir) && Q_stricmp(gamedir, BASEGAME))
	{
		...
		*p_fn = FS_BuildOSPath(homepath, gamedir, fname);
		// copy to destination
		FS_CopyFile(fn, *p_fn);
		if(access(*p_fn, R_OK) == -1)
		{						// could do with FS_CopyFile returning a boolean actually
		...
		if(access(fn, R_OK) == 0)
		{
			...
			if(Q_stricmp(Cvar_VariableString("version"), lastVersion->string))
			{
				...
				if(access(fn, R_OK) == -1)
				{
					...
					{
						Com_Error(ERR_FATAL,
								  "Sys_LoadDll(%s) failed, no corresponding .so file found in fs_homepath or fs_basepath\n",
								  name);
					}
				}
			}
